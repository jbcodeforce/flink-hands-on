...
resource "null_resource" "pre_cleanup_cdc_schemas" {
  ...
  triggers = {
    prefix = var.prefix
    schema_registry_endpoint = data.confluent_schema_registry_cluster.cdc_sr.rest_endpoint
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      SR_ENDPOINT="${data.confluent_schema_registry_cluster.cdc_sr.rest_endpoint}"
...
resource "confluent_connector" "postgres_cdc_source" {
  ...
  config_nonsensitive = {
    ...
    "kafka.service.account.id" = local.connectors_sa.id

    # Database connection
    "database.hostname"    = aws_db_instance.postgres_db.address
    "database.port"        = tostring(aws_db_instance.postgres_db.port)
